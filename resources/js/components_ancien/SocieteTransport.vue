                            ...
                            <tbody>
                                <tr v-for="societeTransport in paginatedSocieteTransports" :key="societeTransport.id">
                                    ...
                                    <td>
                                        ...
                                        <button v-else class="btn btn-danger" @click="quitSociete()">  <i class="ti ti-indent-decrease"></i></button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
  ...
  computed : {
    ...
    filteredSociete(){
        ...
        if (this.search.name !== '') {
            ...
            societeTransport.name.toLowerCase().startsWith(this.search.name.toLowerCase())
        );
        }
      // Filtrer par adresse
      if (this.search.adresse !== '') {
            filtered = filtered.filter(societeTransport =>
            societeTransport.adresse.toLowerCase().startsWith(this.search.adresse.toLowerCase())
        );
        }
...
.row.m-3 .col-8 input.form-control,
.row.m-3 .col-8 select.form-control {
  font-size: 13px; 
}
.spinner-container {
    position: fixed;
