<template>
  <div class="container-fluid">
          ...
          <div class="card-header" style="position:static;">
            ...
            <div class="card-tools">
              ...
              <!-- <button  v-if="!$gate.isLocalStore()" @click="addProductApi()" class="btn btn-light"  style="color: #01356F;">
                Nouvel Api <i class="fa-solid fa-plus"></i>
              </button> -->
            </div>  
          </div>
              ...
              <tbody>
                <tr  v-if="!$store.state.isLoading" v-for="product in paginatedProducts" :key="product.id">
                  ...
                  <td >
                    ...
                    <span v-if="product.status === 'publish'" class="badge badge-success rounded-3 fw-semibold me-2 m-1" style="font-size: 12px;">{{ product.status }} </span>
                    <span v-if="product.status === 'draft'" class="badge badge-warning rounded-3 fw-semibold me-2 m-1" style="font-size: 12px; color: white;">{{ product.status }} </span>
                    <span v-if="product.status === 'pending'" class="badge badge-info rounded-3 fw-semibold me-2 m-1" style="font-size: 12px;">{{ product.status }} </span>
                  </td> 
                  <td style="max-width: 15px; overflow-wrap: break-word;">{{ product.sku }}</td>
                  <td >{{ product.stock_status }} </td>
                  <td >{{ product.price | prix }}  </td>
                  <td >{{ product.stock_quantity }} </td>
                  <td>
                    <a class="btn" @click="showCategories(product)" style="background-color: rgb(179, 178, 178);">
                      <i class="fa fa-eye white" ></i>
                    </a>
                  </td>
                  <td v-if="$gate.isLocalStore()">
                    <div class="badge  rounded-3 fw-semibold me-2 m-1" style="background-color: #E96423; color: white;list-style-type: none; font-size: 12px;">
                      {{ product.marques.name}}
                    </div>
                  </td>
                  <td v-else>
                    <div class="d-flex flex-wrap w-100" >
                      <ul v-for="(attribute, attributeIndex) in product.attributes" :key="attributeIndex"  class="badge  rounded-3 fw-semibold me-2 m-1"  style="background-color: #E96423; color: white;list-style-type: none; font-size: 12px;">
                  ...
                  <td>
                      <button @click="editProduct(product)" class="btn btn-info"><i class="fa-solid fa-edit fa-fw"></i></button>
                      <button @click="deleteProduct(product)" class="btn btn-danger"><i class="fa-sharp fa-solid fa-trash"></i></button>
                  </td>
                </tr>
          ...
          <form id="produit-form" @submit.prevent="editMode ? updateProduct() : createProduct()">
            <div class="modal-body">
              <div class="form-group">
                ...
                <input type="text" class="form-control" id="name" v-model="form.name" placeholder="Nom du produit">
              </div>
              ...
              <div class="form-group">
                ...
                <input type="text" class="form-control" id="sku" v-model="form.sku" placeholder="Entrer la reference">
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <input type="text" class="form-control" id="slug" v-model="form.slug" placeholder="Entrer le permalien">
              </div>
    ...
    <!-- Category Modal -->
    <div >
      <div class="modal fade" id="showCategorie_modal" tabindex="-1" role="dialog">
                ...
                <div class="row">
                  <div class="form-group">
                    <table class="table table-hover text-nowrap">
                      ...
                      <tbody>
                        <tr v-for="categorie in productCategories" :key="categorie.id">
...
export default {
  ...
  methods:{
    ...
    showCategories(product){
      ...
      $('#showCategorie_modal').modal('show');
    },
