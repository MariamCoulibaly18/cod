          ...
          <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
              ...
              <div class="modal-footer">
                <button type="button" class="btn btn-primary rounded-pill px-4" @click="updatePointVente()" :disabled="pointVenteToModify.type_echange_commercial === ''">Modifier</button>
              </div>
            </div>
          </div>
      </div>  
...
export default {
  ...
  filteredPointVentes() {
    ...
    if (this.search.client && this.search.client !== '') {
        ...
        return pointVente.client.toLowerCase().includes(this.search.client.toLowerCase());
      });
    }
    ...
    if (this.search.ville && this.search.ville !== '') {
        filteredPointVentes = filteredPointVentes.filter((pointVente) => {
        return pointVente.ville.toLowerCase().includes(this.search.ville.toLowerCase());
      });
    }

    if (this.search.type_payement && this.search.type_payement !== '') {
        filteredPointVentes = filteredPointVentes.filter((pointVente) => {
        return pointVente.type_payement.toLowerCase().includes(this.search.type_payement.toLowerCase());
      });
    }
