    ...
    <div class="card">
      <div class="card-header">
        ...
        <div class="card-tools">
          <button v-if="$gate.isSuperAdmin()" class="btn btn-light" @click="addModal" style="color: #01356F;" >Nouveau <i class="fa-solid fa-plus"></i></button>
        </div>
    </div>
      ...
      <table class="table table-hover text-nowrap">
        ...
        <tbody>
            <tr v-if="!isLoading" v-for="store in paginatedStores" :key="store.id">
                <td>{{store.id}}</td>
                <td>{{store.name}}</td>
                <td><img :src="store.type.icon" alt="" width="30px" height="30px"></td>
                ...
                <td>
                    <span v-if="!$store.getters.currentBoutique || $store.getters.currentBoutique.id != store.id">
                      <button v-if="connect_id != store.id"  class="btn btn-success" @click="enterStore(store)"> <i class="fa fa-sign-in-alt white"></i></button>
                      <button v-if="connect_id === store.id && isConnecting" class="btn btn-success">
                        <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                        Connecting...
                      </button>
                      <button v-if="$gate.isResponsible(store) || $gate.isSuperAdmin()" class="btn btn-info" @click="editModal(store)"> <i class="fa fa-edit white"></i></button>
                      <button v-if="$gate.isSuperAdmin()" class="btn btn-danger" @click="deleteStore(store.id)"> <i class="fa fa-trash white"></i></button>
                    </span>
                    <button v-else class="btn btn-danger" @click="quitStore()"> <i class="fa fa-sign-out-alt white"></i></button>
                </td>
            </tr>
        </tbody>
      </table>
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
          ...
          <input type="file" name="logo"
            placeholder="Logo" ref="logo"
            class="form-control" :class="{ 'is-invalid': form.errors.has('logo') }" @change="updateLogo">
        ...
        <div v-show="$gate.isSuperAdmin()" class="form-group">
          ...
          <select v-model="form.user.id" type="text" id="user" name="user"
            class="form-select" :class="{ 'is-invalid': form.errors.has('user.id') }">
      ...
      <div class="modal-footer justify-content-center">
        <button type="button" class="btn btn-danger rounded-pill px-4" data-bs-dismiss="modal" @click="closeModal">Annuler</button>
        <button type="submit" class="btn btn-primary rounded-pill px-4" v-if="!editMode">Ajouter </button>
        <button type="submit" class="btn btn-primary rounded-pill px-4" v-if="editMode">Mondifier</button>
      </div>
    </form>
    </div>
  </div>
...
methods:{
 ...
 changePage(page){
  ...
  loadStores(){
    ...
    .then(({data}) => {
      this.stores = data.data.map(store => {
            // Assurez-vous que store_type.icon contient l'URL complète de l'image
            store.type.icon = `${store.type.icon}`;
            return store;
        });
  ...
  //   .then(({data}) => {
  //       this.store_types = data.data.map(store_type => {
  //           // Assurez-vous que store_type.icon contient l'URL complète de l'image
  //           store_type.icon = `${store_type.icon}`;
  //           return store_type;
  //       });
