 ...
 <div class="col-md-12">
    ...
    <div class="card-header bg-dark">
      ...
      <div class="card-tools">
        <button class="btn btn-light" @click="addModal"  style="color: #01356F; "  >Nouveau <i class="fa-solid fa-plus"></i></button>
      </div>
    </div>
      ...
      <table class="table table-hover text-nowrap">
        ...
        <tbody>
            <tr v-for="store_type in paginatedStoreTypes" :key="store_type.id">
                ...
                <td><img :src="store_type.icon" alt="" width="50" height="50"></td>
                <td>{{store_type.name}}</td>
                <td>{{store_type.description}}</td>
                <td>
                  <button class="btn btn-info" @click="editModal(store_type)"><i class="fa fa-edit"></i></button>
                  <button class="btn btn-danger" @click="deleteStoreType(store_type.id)"><i class="fa fa-trash"></i></button>
                </td>
            </tr>
        </tbody>
      </table>
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-primary rounded-pill px-4" v-if="editMode"> <i class="fa fa-edit white"></i></button>
      </div>
    </form>
    </div>
  </div>
  ...
  export default {
      data: () => ({
       ...
       form: new Form({
         id:'',
         name:'',
         icon: null,  // Nouveau champ pour stocker l'image
         description:'',
      }),
      search : {
        name:'',
...
methods:{
 ...
 changePage(page){
  ...
  createStoreType(){
    ...
    axios.post('/api/boutique_type', formData, {
        headers: {
...
.row.mt-3 .col-4 input.form-control{
  font-size: 13px; 
}
.spinner-container {
    position: fixed;
